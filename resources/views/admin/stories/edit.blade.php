    ...
    <div class="py-8 max-w-2xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('admin.stories.update', $story) }}" enctype="multipart/form-data" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title_en" value="{{ old('title_en', $story->title_en) }}" class="w-full border-gray-300 rounded-md">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" class="w-full border-gray-300 rounded-md">{{ old('description', $story->description) }}</textarea>
                <x-input-error :messages="$errors->get('description')" class="mt-1" />
            </div>

            <div>
                <label class="block text-sm font-medium mb-1 text-gray-500">คำอธิบาย (English)</label>
                <textarea name="description_en" rows="5" class="w-full border-gray-300 rounded-md">{{ old('description_en', $story->description_en) }}</textarea>
            ...
            <div>
                ...
                @if ($story->video)
                    <div class="flex items-center gap-3 mb-2">
                        <video src="{{ $story->video_url }}" class="w-40 rounded-md border" controls></video>
