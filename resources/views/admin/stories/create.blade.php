<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">โพสต์เรื่องราวใหม่</h2>
    </x-slot>

    <div class="py-8 max-w-2xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('admin.stories.store') }}" enctype="multipart/form-data" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
            @csrf
            ...
            <div>
                <label class="block text-sm font-medium mb-1 text-gray-500">หัวข้อ (English)</label>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" class="w-full border-gray-300 rounded-md">{{ old('description') }}</textarea>
                <x-input-error :messages="$errors->get('description')" class="mt-1" />
            </div>

            <div>
                <label class="block text-sm font-medium mb-1 text-gray-500">คำอธิบาย (English)</label>
                <textarea name="description_en" rows="5" class="w-full border-gray-300 rounded-md">{{ old('description_en') }}</textarea>
                <x-input-error :messages="$errors->get('description_en')" class="mt-1" />
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">รูปภาพ (เลือกได้หลายรูป)</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="starts_on" value="{{ old('starts_on') }}" class="w-full border-gray-300 rounded-md">
                ...
                <div>
                    ...
                    <input type="date" name="ends_on" value="{{ old('ends_on') }}" class="w-full border-gray-300 rounded-md">
